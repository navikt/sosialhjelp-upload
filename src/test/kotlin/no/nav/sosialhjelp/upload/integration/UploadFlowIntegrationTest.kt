...
class UploadFlowIntegrationTest {
     ...
     */
    private fun appTest(block: suspend ApplicationTestBuilder.() -> Unit) =
        testApplication {
    ...
    private suspend fun tusUpload(
        ...
        val createResp =
            client.post("/sosialhjelp/upload/tus/files") {
                ...
                header(
                    ...
                    "filename ${base64(
                        filename,
                    )}, contextId ${base64(contextId)}, fiksDigisosId ${base64(UUID.randomUUID().toString())}",
                )
            }
        ...
        val patchResp =
            client.patch("/sosialhjelp/upload/tus/files/$uploadId") {
                header("Authorization", "Bearer $token")
                header("Tus-Resumable", "1.0.0")
    ...
    @Test
    fun `TUS upload stores file in mellomlagring`() =
        appTest {
    ...
    @Test
    fun `deleting uploaded file removes from DB and calls mellomlagring deleteFile`() =
        appTest {
            val contextId = UUID.randomUUID().toString()
            val token = JwtTestUtils.issueToken()
            val filId = UUID.randomUUID()
    ...
    @Test
    fun `submit calls Fiks and deletes submission from DB`() =
        appTest {
            val contextId = UUID.randomUUID().toString()
            val token = JwtTestUtils.issueToken()
            val filId = UUID.randomUUID()
            ...
            val submitResp =
                client.post("/sosialhjelp/upload/submission/$submissionId/submit") {
                    header("Authorization", "Bearer $token")
    ...
    @Test
    fun `submit returns 201 and cleans up when Fiks reports ettersendelse already exists`() =
        appTest {
            val contextId = UUID.randomUUID().toString()
            val token = JwtTestUtils.issueToken()
            val filId = UUID.randomUUID()
    ...
    @Test
    fun `POST to contextId owned by another user returns Forbidden`() =
        appTest {
            val contextId = UUID.randomUUID().toString()
    ...
    @Test
    fun `SSE stream emits updated state after TUS upload completes`() =
        appTestReal { client ->
            val contextId = UUID.randomUUID().toString()
            val token = JwtTestUtils.issueToken()
            val filId = UUID.randomUUID()
    ...
    @Test
    fun `SSE stream reflects deleted upload`() =
        appTestReal { client ->
            val contextId = UUID.randomUUID().toString()
            val token = JwtTestUtils.issueToken()
            val filId = UUID.randomUUID()
    ...
    @Test
    fun `after submit and reconnect, new SSE stream receives updates for new uploads`() =
        appTestReal { client ->
            val contextId = UUID.randomUUID().toString()
            val token = JwtTestUtils.issueToken()
            val fiksDigisosId = UUID.randomUUID().toString()
            ...
            val mockSak = mockk<DigisosSak> { every { kommunenummer } returns "0301" }
            coEvery { fiksClient.getSak(fiksDigisosId, any()) } returns mockSak
            coEvery { fiksClient.uploadEttersendelse(any(), any(), any(), any(), any(), any()) } returns
                mockk<HttpResponse> {
                    every { status } returns HttpStatusCode.OK
                }
    ...
    @Test
    fun `submit returns 201 when Fiks rejects duplicate file names with 400`() =
        appTest {
            val contextId = UUID.randomUUID().toString()
            val token = JwtTestUtils.issueToken()
            val fiksDigisosId = UUID.randomUUID().toString()
            ...
            coEvery { fiksClient.uploadEttersendelse(any(), any(), any(), any(), any(), any()) } answers {
                ...
                mockk<HttpResponse> {
                    every { status } returns
                        if (filenames.size != filenames.distinct().size) {
    ...
    @Test
    fun `SSE endpoint responds with event-stream content type`() =
        appTestReal { client ->
            val contextId = UUID.randomUUID().toString()
