...
class VirusScannerTest {
    ...
    @Test
    fun `returns OK when scanner reports no virus`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .newBuilder()
    ...
    @Test
    fun `returns FOUND when scanner reports virus`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .newBuilder()
    ...
    @Test
    fun `returns ERROR when scanner returns HTTP 500`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .newBuilder()
    ...
    @Test
    fun `returns ERROR when scanner is unreachable`() =
        runTest {
    ...
    @Test
    fun `returns OK when URL is empty (scanner not configured)`() =
        runTest {
