...
class TexasClientTest {
    ...
    @Test
    fun `returns access token on success`() =
        runTest {
    ...
    @Test
    fun `caches token and does not make second HTTP request within expiry`() =
        runTest {
    ...
    @Test
    fun `fetches new token after expiry`() =
        runTest {
    ...
    @Test
    fun `throws when server returns error`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .newBuilder()
